...
- `tts_voice` (uint8_t): eSpeak-NG voice index (0=en, 1=en-US, 2=en-GB-x-rp, 3=en-GB-scotland). JS bindings: `tts_set_voice(name)` / `tts_get_voice()`. Voice setting is hidden in UI when Flite engine is active. Persisted in tts.json; synced back to shared memory on boot.
